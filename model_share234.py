...
class base_resnet(nn.Module):
    ...
    def forward(self, x):
        x = self.base.layer1(x) ##从stage3开始共享参数
...
class embed_net(nn.Module):
    ...
    def forward(self, x1, x2, modal=0):
        if modal == 0:
            ...
            gray = F.relu(self.decode(torch.cat((gray1, gray2),0))) # 16 3 H W
            gray1, gray2 = torch.chunk(gray, 2, 0) #  16 3 H w 
        ...
        #############################切出每个模态
        rgb, m_rgb, ir ,m_ir = torch.chunk(x,4,0) # every modality 16 2048 24 8
        # pool
        rgb   = self.avgpool(rgb)
        m_rgb = self.avgpool(m_rgb)
        ir    = self.avgpool(ir)
        m_ir  = self.avgpool(m_ir)
        # view 
        rgb = rgb.view(rgb.size(0), rgb.size(1)) # 16 2048
        m_rgb = m_rgb.view(m_rgb.size(0), m_rgb.size(1))
        ir = ir.view(ir.size(0), ir.size(1))
        m_ir = m_ir.view(m_ir.size(0), m_ir.size(1))
        ...
        x41 = x41.view(x41.size(0), x41.size(1)) # 64 2048
        ...
        if self.training:
            return x41, x42, x43, x44, self.classifier1(feat41), self.classifier2(feat42), self.classifier3(feat43), self.classifier4(feat44), rgb,m_rgb,ir,m_ir, [xo, gray]
        else:
            return self.l2norm(torch.cat((x41, x42, x43, x44),1)), self.l2norm(torch.cat((feat41, feat42, feat43, feat44),1))
            
        ####################################################################
        # 返回全局特征 与train_my_center_loss.py  train_my_center_hctriloss.py 对应 共享了2 3 4层网络
        ##################################################################
