...
class embed_net(nn.Module):
    def __init__(self,  class_num , n_parts, no_local= 'on', gm_pool = 'on', arch='resnet50'):
        ...
        self.global_classifier = nn.Linear(pool_dim , n_parts * 3 , bias=False)
    ...
    def forward(self, x1, x2 , modal=0):
        ...
        # print("avgpool" , global_feature.shape)
        global_feature = self.global_bottleneck(global_feature)
        # print("bottleneck" , global_feature.shape)
        global_feature = self.global_classifier(global_feature)
        global_feature = F.relu(global_feature)
        # print("classifier" , global_feature.shape)
        global_feature = global_feature.view(-1 , self.n_parts , 3)
        # print("view" , global_feature.shape)
        jets = torch.chunk(global_feature , 4 , 0)
        jet1 , jet2 , jet3 , jet4 = jets
        ...
        if self.training:
            return (*x_parts, *outputs, rgb , m_rgb , ir , m_ir , jet1 , jet2 , jet3 , jet4 , [xo, gray])
        else:
