...
class embed_net(nn.Module):
    ...
    def forward(self, x1, x2 , modal=0):
        ...
        all_feats = [self.avgpool(x_part) for x_part in torch.chunk(x, 3, 0)]
        all_feats = [x_part.view(x_part.size(0) , x_part.size(1)) for x_part in all_feats]
        ...
        if self.training:
            return (*x_parts, *outputs, *all_feats )
        else:
