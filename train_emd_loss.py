...
from model_emd import embed_net
from utils import *
...
parser.add_argument('--n_parts', default=10, type=int, help='parameter of emd loss , should less than 32')
...
if args.method =='base':
    net = embed_net(n_class , args.n_parts , no_local= 'off', gm_pool =  'off', arch=args.arch)
else:
    net = embed_net(n_class , args.n_parts , no_local= 'on', gm_pool = 'on', arch=args.arch)
net.to(device)
...
criterion_emd = EMDLoss(n_parts=args.n_parts)

criterion_id.to(device)
criterion_tri.to(device)
criterion_div.to(device)
criterion_emd.to(device)
...
if args.optim == 'sgd':
    ...
    ignored_params += list(map(id, net.global_bottleneck.parameters()))
    ignored_params += list(map(id, net.global_classifier.parameters()))
...
def train(epoch):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        feat1, feat2, feat3, feat4, out1, out2, out3, out4, rgb , m_rgb , ir , m_ir , jet1 , jet2 , jet3 , jet4 , g1 = net(input1, input2)
        ...
        # print("jet1" , jet1.shape)
        # print("n_parts" , args.n_parts)
        loss_emd = criterion_emd(jet1,jet2) + criterion_emd(jet1,jet3) +criterion_emd(jet1,jet4) 
        # print("loss_emd" , loss_emd.shape)
        loss_emd += (criterion_emd(jet2,jet3) + criterion_emd(jet2,jet4))
        loss_emd += criterion_emd(jet3 , jet4)
        loss_emd /= 6
        loss_emd = loss_emd.sum()

        correct += (ba / 2)
        _, predicted = out1.max(1)
        correct += (predicted.eq(labels).sum().item() / 2)

        loss = loss_id + loss_tri + loss_dcl + loss_emd
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        emd_loss.update(loss_emd.item() , 2 * input1.size(0))
        total += labels.size(0)
