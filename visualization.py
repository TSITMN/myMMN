...
from visualization_model import embed_net
from utils import *
...
parser.add_argument('--model_path', default='save_model_ablation/', type=str, help='model save path')
parser.add_argument('--save_epoch', default=20, type=int, metavar='s', help='save model every 10 epochs')
parser.add_argument('--log_path', default='log_ablation/', type=str, help='log save path')
parser.add_argument('--vis_log_path', default='log_ablation/vis_log/', type=str, help='log save path')
...
start_epoch = 0
args.resume = 'sysu_indoor_encoder3_share1_caloss_agw_p4_n4_lr_0.1_seed_0_best.t'
...
def save_images(tensor, directory, prefix="image", format='jpg'):
    # Ensure the directory exists
    if not os.path.exists(directory):
        os.makedirs(directory)

    # Select the first channel for visualization
    # Change this if you want to visualize different channels or a combination of channels
    tensor = tensor[:, 0, :, :]

    # Convert the tensor to a PIL Image and save it
    for i, img_tensor in enumerate(tensor):
        img_tensor = img_tensor.unsqueeze(0)  # Add back the channel dimension
        img = ToPILImage()(img_tensor)
        file_path = os.path.join(directory, f"{prefix}_{i+1:04d}.{format}")
        img.save(file_path)

# Example usage:
# Assuming 'images_tensor' is your 4D tensor of shape [N, 384, H, W]
# save_images(images_tensor, './saved_images', 'my_prefix')
...
def train(epoch):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        save_images(input1 , directory='./visualization' , prefix='input1')
        save_images(input2 , directory='./visualization' , prefix='input2')
